      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="mb-8">
          <div className="inline-block mb-3 px-4 py-1 rounded-full bg-primary-500/10 border border-primary-500/20">
            <span className="text-primary-400 text-sm font-semibold">Smart Contract Docs</span>
          </div>
          <h1 className="text-3xl font-mono font-bold mb-4 text-primary-400">Smart Contract Documentation</h1>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="flex flex-col space-y-4">
            <div className="bg-gray-900/50 rounded-lg border border-gray-800 hover:border-primary-500/30 transition-colors duration-300 shadow-lg">
              <div className="p-4 border-b border-gray-800 flex items-center gap-2">
                <FileText className="text-primary-400" size={20} weight="duotone" />
              ...
              <textarea
                ...
                className="w-full h-[400px] bg-transparent p-6 font-mono text-sm resize-none focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500/50 transition-all duration-200 text-white custom-scrollbar"
              />
            </div>
            <button
            ...
            <div className="flex-1 bg-gray-900/50 rounded-lg border border-gray-800 hover:border-primary-500/30 transition-colors duration-300 shadow-lg">
              <div className="p-4 border-b border-gray-800 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <Article className="text-primary-400" size={20} weight="duotone" />
              ...
              <div className="h-[600px] overflow-auto p-6 custom-scrollbar">
                {documentation ? (
                  <div className="space-y-6">
                    <div>
                      <h2 className="text-2xl font-bold mb-2 text-primary-400">{documentation.name}</h2>
                      <p className="text-gray-300">{documentation.description}</p>
                      <div className="flex gap-4 mt-3">
                        <span className="text-sm bg-primary-500/10 text-primary-300 px-2 py-0.5 rounded-full border border-primary-500/20">v{documentation.version}</span>
                        <span className="text-sm bg-primary-500/10 text-primary-300 px-2 py-0.5 rounded-full border border-primary-500/20">{documentation.license} License</span>
                      </div>
                    </div>
                    ...
                    {documentation.functions?.length ? (
                      <div>
                        <div className="flex items-center gap-2 mb-4">
                          <FunctionIcon className="text-primary-400" size={20} weight="duotone" />
                          <h3 className="text-lg font-semibold text-primary-400">Functions</h3>
                        </div>
                        <div className="space-y-4">
                          {documentation.functions.map((func, index) => (
                            <div key={index} className="bg-gray-800/70 rounded-lg p-4 border border-gray-700/70 hover:border-primary-500/30 transition-colors duration-200">
                              <div className="flex items-center gap-2 mb-2">
                                <span className="font-mono text-primary-400">{func.name}</span>
                                <span className="text-sm px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-300 border border-primary-500/20">{func.visibility}</span>
                              </div>
                              <p className="text-sm text-gray-300 mb-3">{func.description}</p>
                              {func.params?.length > 0 && (
                                <div className="mt-2 bg-gray-900/50 rounded p-3">
                                  <div className="text-sm text-primary-300 mb-2">Parameters:</div>
                    ...
                    {(documentation.events?.length ?? 0) > 0 && (
                      <div>
                        <div className="flex items-center gap-2 mb-4">
                          <Bell className="text-primary-400" size={20} weight="duotone" />
                          <h3 className="text-lg font-semibold text-primary-400">Events</h3>
                        </div>
                        <div className="space-y-4">
                          {documentation.events?.map((event, index) => (
                            <div key={index} className="bg-gray-800/70 rounded-lg p-4 border border-gray-700/70 hover:border-primary-500/30 transition-colors duration-200">
                              <div className="font-mono text-primary-400 mb-2">{event.name}</div>
                    ...
                    {(documentation.variables?.length ?? 0) > 0 && (
                      <div>
                        <div className="flex items-center gap-2 mb-4">
                          <Database className="text-primary-400" size={20} weight="duotone" />
                          <h3 className="text-lg font-semibold text-primary-400">State Variables</h3>
                        </div>
                        <div className="space-y-4">
                          {documentation.variables?.map((variable, index) => (
                            <div key={index} className="bg-gray-800/70 rounded-lg p-4 border border-gray-700/70 hover:border-primary-500/30 transition-colors duration-200">
                              <div className="flex items-center gap-2 mb-2">
                                <span className="font-mono text-primary-400">{variable.name}</span>
                                <div className="flex space-x-2">
                                  <span className="text-sm px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-300 border border-primary-500/20">{variable.type}</span>
                                  <span className="text-sm px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-300 border border-primary-500/20">{variable.visibility}</span>
                                </div>
                              </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-gray-400">
                    <div className="relative">
                      ...
                      <BookOpen size={64} className="text-primary-400 relative z-10 mb-6" weight="duotone" />
                    </div>
